...
export default function ContractorLayout() {
  ...
  const navigation = [
    ...
    { name: 'Tasks', href: '/contractor/tasks', icon: CheckSquare },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="flex">
        ...
        <aside className={`
          ...
          w-64 bg-gradient-to-b from-orange-600 to-orange-800 shadow-lg
            ...
            <div className="p-4 lg:p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs lg:text-sm text-orange-100 mt-1">Contractor</p>
                </div>
                <button
                  onClick={() => setSidebarOpen(false)}
            ...
            <nav className="flex-1 mt-4 lg:mt-8 overflow-y-auto">
              {navigation.map((item) => {
                const isActive = location.pathname === item.href || 
                  (item.href !== '/contractor' && location.pathname.startsWith(item.href))
                return (
                  <button
                    key={item.name}
                    onClick={() => {
                    ...
                    className={`w-full flex items-center px-4 lg:px-6 py-3 text-left text-orange-100 hover:bg-orange-700 transition-colors ${
                      isActive ? 'bg-orange-700 border-r-4 border-white' : ''
                    }`}
                  >
